...
function checkGhCli() {
    try {
        ...
        console.error(chalk.red('Github Cli (GH) was not found. Install it: https://cli.github.com/'));
        process.exit(1);
    }
}
    ...
    function inner() {
        return (tree) => {
            ...
            for (let i = 0; i < nodes.length; i++) {
                ...
                if (foundVersion.type !== 'text')
                    continue;
                const versionText = foundVersion.value.trim();
                if (!semver.valid(versionText))
                    continue;
                if (!semver.eq(versionText, version))
                    continue;
...
async function performGitOperations(version, branch) {
    try {
        ...
        execCommand(`git commit -m 'chore: update plugin version to ${version}'`, {
            stdio: 'ignore',
        });
...
export async function release(isDryRunOption) {
    ...
    while (true) {
        ...
        if (isDryRun) {
            console.log(chalk.blue(`[DRY RUN] Version would be updated to ${RELEASE_VERSION} in package.json and manifest.json`));
        }
        else {
        ...
        if (!MAIN_BRANCHES.includes(currentBranch)) {
            console.log(chalk.red(`Expected one of branches: ${MAIN_BRANCHES.join(', ')}, got branch: ${currentBranch}`));
            if (!isDryRun) {
                process.exit(1);
            }
        }
        ...
        if (isDryRun) {
            console.log(chalk.blue(`🔍 DRY RUN COMPLETE: Release ${RELEASE_VERSION} would be successfully created and published!`));
        }
        else {
            console.log(chalk.green(`Release ${RELEASE_VERSION} has been successfully created and published!`));
        }
        break;
    }
}
